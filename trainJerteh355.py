...
REP_DIR = os.path.join(ROOT_DIR, "reports", "Jerteh355")
maxlen = 300
# Create directory if not exists
if not os.path.exists(REP_DIR):
    os.makedirs(REP_DIR)
def train_model (i, polarity, eval = "f1", epochs=16):
    ...
    model_name = f"jerteh/Jerteh-355"
    ...
    outputdir = f"Jerteh355SENT{polarity}{i}"
...
def test_model(i, polarity):
    ...
    model_name = f"Tanor/Jerteh355SENT{polarity}{i}"
...
def test_model_local(i, polarity):
    ...
    X_test = X_test.fillna("")    
    # Construct model name
    model_name = f"Jerteh355SENT{polarity}{i}"
...
def upload_local_model_to_hub(i, polarity):
    ...
    model_name_local = f"Jerteh355SENT{polarity}{i}"
    model_name_hub = f"Tanor/Jerteh355SENT{polarity}{i}"
